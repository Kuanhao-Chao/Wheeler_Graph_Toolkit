...
void digraph::relabel_initialization() {
    ...
    for (auto& [edge_label, edges] : _edge_label_2_edge) {
        ...
        for (auto& edge: edges) {
            this -> relabel_by_node_address(edge.get_head(), accum_label);
        }
    }
...
void digraph::one_scan_through_wg_rg(string label) {
    ...
    end_node_label = prev(_edge_label_2_edge[label].end()) -> get_head_label();
#ifdef DEBUGPRINT
...
void digraph::permutation_start(bool early_stop) {
    ...
    do {
        ...
        this -> permutation_4_edge_group(this -> get_first_edge_label(), early_stop);
...
void digraph::permutation_4_edge_group(string label, bool early_stop) {
    ...
    do{
        if (itv != _edge_label_2_edge[label].end()) {
            if (prev_num == itv->get_head_label() &&  prev_address == itv->get_head()) {
#ifdef DEBUGPRINT
                cout << "They are the same node. No accumulation." << endl;
#endif
            } else if (prev_num == itv->get_head_label() &&  prev_address != itv->get_head()) {
...
void digraph::permutation_4_sub_edge_group(string &label, vector<int> &prev_num_vec, vector<int> &accum_same_vec, map<int, vector<int*> > &nodes_2_relabelled_nodes_vec, int index, bool early_stop) {
    ...
    if (index < prev_num_vec.size() && index < accum_same_vec.size()) {
        ...
        do {
            ...
            bool WG_valid = true;
            WG_valid = this -> WG_checker_in_edge_group(label, _edge_label_2_edge[label]);
#ifdef DEBUGPRINT
        ...
        if (this -> get_next_edge_label(label)  != "" ) {
            ...
            if (WG_valid) {
                ...
                this -> invalid_wheeler_graph("Wrong", false);
            }
        }
    }
#ifdef DEBUGPRINT
