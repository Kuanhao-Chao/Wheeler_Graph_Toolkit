...
digraph::digraph(vector<string> node_names, int nodes_num, int edges_num, string path_name) {
    ...
    // Engineering way. Just create double size first.
    _node_ptrs = new int[_nodes_num*10];
    _prev_node_ptrs = new int[_nodes_num*10];
    ...
    ********************************/
    ...
    for(auto node : node_names) {
        _node_2_ptr_address[this->get_encoded_nodeName(node)] = &_node_ptrs[_node_itr_counter];
        _prev_node_2_ptr_address[this->get_encoded_nodeName(node)] = &_prev_node_ptrs[_node_itr_counter];
        _node_ptrs[_node_itr_counter] = _node_itr_counter + 1; 
        _prev_node_ptrs[_node_itr_counter] = _node_itr_counter + 1; 
        _node_itr_counter = _node_itr_counter+1;
        _node_copy_counter[this->get_encoded_nodeName(node)] = 0;
#ifdef DEBUGPRINT
        cout << "** _node_itr_counter: " << _node_itr_counter-1 << endl;
        ...
        cout << "** _node_ptrs[_node_itr_counter]: " << _node_ptrs[_node_itr_counter-1] << endl;
        cout << "** _prev_node_ptrs[_node_itr_counter]: " << _prev_node_ptrs[_node_itr_counter-1] << endl;
#endif
    }
}
...
void digraph::innodelist_sort_relabel() {
    ...
    while (!prelabels_fixed) {
        ...
        for (auto& [label, edges] : _edgeLabel_2_edge) {
            ...
            do {
                ...
                cout << "****** accum_edgegp_size: " << accum_edgegp_size << endl;
#endif

                edgegp_nodes.clear();
                edgegp_node_2_innodes_vec.clear();
                index.clear();
                for (auto& edge : edges) {
#ifdef DEBUGPRINT
                    edge.print_edge();
                    ...
                    if (find(edgegp_nodes.begin(), edgegp_nodes.end(), edge.get_head_name()) == edgegp_nodes.end()) {
                        ...
                        edgegp_node_2_innodes_vec.push_back(this->get_innodesList_uniq(label, edge.get_head_name()));
                    }
                }
                ...
                ********************************/
                ...
                // Add #edgegp_nodes.size() of 0 into the index vector
                index.insert(index.end(), edgegp_nodes.size(), 0);


                sort_valid = this->in_edge_group_sort(edgegp_nodes, edgegp_node_2_innodes_vec, index, accum_edgegp_size, label);
            } while (!sort_valid);
    ...
    // cout << "All labels are fixed!!!" << endl;
...
vector<int> digraph::get_innodesList_uniq(int edgeLabel, int node_name) {
#ifdef DEBUGPRINT
    cout << "*********************************" << endl;
    cout << "*** get_innodesList_uniq =>  node_name: "  << this->get_decoded_nodeName(node_name) << "  "<< this -> get_node_label(node_name) << endl;
    cout << "*********************************" << endl;
#endif
...
void digraph::get_innodesList(int edgeLabel, int node_name, vector<int>& innodesList, vector<int>& innodes_name) {
#ifdef DEBUGPRINT
    cout << "*********************************" << endl;
    cout << "*** get_innodesList =>  node_name: "  << this->get_decoded_nodeName(node_name) << "  "<< this -> get_node_label(node_name) << endl;
    cout << "*********************************" << endl;
#endif

    // for (auto& innode : _node_2_innodes[node_name]) {
    //     this->print_node(innode);
    //     innodes_name.push_back(innode);
    //     innodesList.push_back(this->get_node_label(innode));
    // }
    ...
    for (auto& edge : _edgeLabel_2_edge[edgeLabel]) {
        if (edge.get_head_name() == node_name) {
                ...
                cout << "edge.get_tail(): " << edge.get_tail() << endl; 
#endif
                innodes_name.push_back(edge.get_tail_name());
                innodesList.push_back(edge.get_tail_label());
        }
    }


    vector<int> indices(innodes_name.size());
    iota(indices.begin(), indices.end(), 0);
    sort(indices.begin(), indices.end(),
        [&](int A, int B) -> bool {
            return innodesList[A] < innodesList[B];
        });


    // cout << "Sorted indices in get_innodesList." << endl;
    // for (auto& i : indices) {
    //     cout << i << "  ";
    // }
    // cout << endl;
    reorder_node_vec(innodesList, indices);
    reorder_node_vec(innodes_name, indices);
}
...
bool digraph::in_edge_group_sort(vector<int> &edgegp_nodes, vector<vector<int> > &edgegp_node_2_innodes_vec, vector<int> &index, int &accum_edgegp_size, int curr_edgegp) {
#ifdef DEBUGPRINT
    cout << "*************************" << endl;
    ...
    sort(index.begin(), index.end(),
        [&](const int& a, const int& b) {
            // Has already failed. => directly end the sorting.
            if (!sort_success) return false;
            if (edgegp_node_2_innodes_vec[a] < edgegp_node_2_innodes_vec[b]) {
                if (edgegp_node_2_innodes_vec[b].front() < edgegp_node_2_innodes_vec[a].back()) {
...
#ifdef DEBUGPRINT
                    cout << "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX (a < b)\n";
                    ...
                    // Since edgegp_node_2_innodes_vec[a] is smaller, split edgegp_node_2_innodes_vec[a].
                    
                    this -> in_edge_group_conflict_fix(edgegp_nodes[a], edgegp_nodes[b], accum_edgegp_size, edgegp_nodes.size(), curr_edgegp);
                    sort_success = false;
                    // this -> invalid_wheeler_graph("", true);
                }
                ...
                if (edgegp_node_2_innodes_vec[a].front() < edgegp_node_2_innodes_vec[b].back()) {
                    ...
                    this -> in_edge_group_conflict_fix(edgegp_nodes[b], edgegp_nodes[a], accum_edgegp_size, edgegp_nodes.size(), curr_edgegp);
                    sort_success = false;
                    // this -> invalid_wheeler_graph("", true);
                }
            }
...
void digraph::in_edge_group_conflict_fix(int& small_node, int& large_node, int &accum_edgegp_size, int curr_edgegp_size, int curr_edgegp) {
    int split_idx = 0;

    // cout << "\n\n>>>> Smaller node!!!" << endl;
    vector<int> small_innodesList;
    vector<int> small_innodes_name;
    this -> get_innodesList(curr_edgegp, small_node, small_innodesList, small_innodes_name);
#ifdef DEBUGPRINT
    cout << "*********************************" << endl;
    cout << "Starting of small_innodesList: " ;
    for (auto& node_label : small_innodesList) {
        cout << node_label << " ";
    }
    cout << "End of small_innodesList " << endl;
    cout << "*********************************" << endl;
#endif

    // cout << "\n\n>>>> Larger node!!!" << endl;
    vector<int> large_innodesList;
    vector<int> large_innodes_name;
    this -> get_innodesList(curr_edgegp, large_node, large_innodesList, large_innodes_name);

#ifdef DEBUGPRINT
    cout << "*********************************" << endl;
    cout << "Starting of large_innodesList: " ;
    for (auto& node_label : large_innodesList) {
        cout << node_label << " ";
    }
    cout << "End of large_innodesList " << endl;
    cout << "*********************************" << endl;
#endif



    for (int i=0; i<small_innodesList.size(); i++) {
        // cout << "i: " << i << endl;
        // cout << "small_innodesList[i]: " << small_innodesList[i] << endl;
        // cout << "large_innodesList[0]: " << large_innodesList[0] << endl;
        if (small_innodesList[i] > large_innodesList[0]) {
            // cout << "## Position found!! " << i << " (" << small_innodesList[i] << ")" << endl;
            split_idx = i;
            break;
        }
    }

    // Create a new node at the split_idx
    string small_node_name = this->get_decoded_nodeName(small_node);
    string delimiter = "_";
    string base_node_name = small_node_name.substr(0, small_node_name.find(delimiter));
    int base_node_name_encode = this -> get_encoded_nodeName(base_node_name);
    _node_copy_counter[base_node_name_encode] += 1;
    string new_node_name = base_node_name + "_COPY" + to_string(_node_copy_counter[base_node_name_encode]);

    int new_node = this->get_encoded_nodeName(new_node_name);
    _node_2_ptr_address[new_node] = &_node_ptrs[_node_itr_counter];
    _prev_node_2_ptr_address[new_node] = &_prev_node_ptrs[_node_itr_counter];
    _node_ptrs[_node_itr_counter] = accum_edgegp_size + curr_edgegp_size + 1 + _root.size(); 
    _prev_node_ptrs[_node_itr_counter] = accum_edgegp_size + curr_edgegp_size + 1 + _root.size(); 
    _node_itr_counter = _node_itr_counter+1;
    _nodes_num += 1;
    _copy_counter += 1;
...
#ifdef DEBUGPRINT
    cout << "#### Before updating innode dictionary" << endl;
    for (auto& [node, innodes] : _node_2_innodes) {
        cout << "## node: " << this -> get_decoded_nodeName(node) << " (" << node << ") " << endl;
        for (auto& innode : innodes) {
            cout << this->get_node_label(innode) << "  " << endl; 
        }
        cout << endl;
    }
#endif

    // !!!! TO-DO need to fix!! Some innode lists are still wrong.!!!
    /***********************************************
    ***  Constructing the innode dictionary for the new small node.
    ************************************************/
    set<int> new_innodes_vc;
    // cout << "@@ Constructing the innode dictionary for the new small node." << endl;
    for (int i=split_idx; i<small_innodesList.size(); i++) {
        // cout << "@@ i: " << i << "  node label: " << small_innodesList[i] << "  node name: " << this -> get_decoded_nodeName(small_innodes_name[i]) << endl;
        new_innodes_vc.insert(small_innodes_name[i]);
    }
    // cout << endl << endl;
    _node_2_innodes[new_node] = new_innodes_vc;

    /***********************************************
    ***  Updating the innode dictionary for the old small node.
    ************************************************/
    set<int> update_innodes_vc;
    // cout << "@@ Updating the innode dictionary for the old small node." << endl;
    for (int i=0; i<split_idx; i++) {
        // cout << "@@ i: " << i << "  node label: " << small_innodesList[i] << "  node name: " << this -> get_decoded_nodeName(small_innodes_name[i]) << endl;
        update_innodes_vc.insert(small_innodes_name[i]);
    }
    _node_2_innodes[small_node] = update_innodes_vc;

#ifdef DEBUGPRINT
    cout << "#### After updating innode dictionary" << endl;
    for (auto& [node, innodes] : _node_2_innodes) {
        cout << "## node: " << this -> get_decoded_nodeName(node) << " (" << node << ") " << endl;
        for (auto& innode : innodes) {
            cout << this->get_node_label(innode) << "  " << endl; 
    ...
    /***********************************************
    ***  Updating _edgeLabel_2_edge map.
    ************************************************/
    map<int, vector<edge> > e_new_outs;
    // map<int, edge> replace_e_new_in;
    for (auto& [label, edges] : _edgeLabel_2_edge) {
        // for (auto& edge_itr : edges) {
        // for (vector<edge>::iterator edge_itr=edges.begin(); edge_itr != edges.end(); edge_itr++) {

        for (int eidx=0; eidx<edges.size(); eidx++) {
            edge edge_itr = edges[eidx];

            if (edge_itr.get_head_name() == small_node) {
                /***********************************************
                ***  These are in-edges to small_node! (split the innodes[in-edges]) 
                ***    (1) Some of them need to be removed.
                ***    (2) Need to add the removed one to the new node!
                ************************************************/
                //  If the tail_name is in the new_innodes_vc => they need to be moved to the new node!!
                // cout << "## These are in-edges to small_node! " << endl;
                // edge_itr.print_edge();
                int tail_node = edge_itr.get_tail_name();
                bool move_2_new_node = (new_innodes_vc.find(tail_node) != new_innodes_vc.end());
                // cout << "move_2_new_node: " << move_2_new_node << endl;

                if (move_2_new_node) {
                    edge e_new_in = edge(curr_edgegp, tail_node, this->get_node_address(tail_node), new_node, this->get_node_address(new_node));
                    edges[eidx] = e_new_in;
                    // replace_e_new_in[eidx] = e_new_in;
                }

            } else if (edge_itr.get_tail_name() == small_node) {
                /***********************************************
                ***  These are out-edges of small_node! They need to be duplicated to the new node. (repeat the out-edges)
                ************************************************/
                // cout << "## These are out-edges of small_node! " << endl;
                // edge_itr.print_edge();
                int head_node = edge_itr.get_head_name();
                int label_grp = edge_itr.get_label();
                edge e_new_out = edge(label_grp, new_node, this->get_node_address(new_node), head_node, this->get_node_address(head_node));

                if (e_new_outs.find(label_grp) == e_new_outs.end()) {
                    // The key is not found.
                    vector<edge> es;
                    es.push_back(e_new_out);
                    e_new_outs[label_grp] = es;
                } else {
                    // The key is found.
                    e_new_outs[label_grp].push_back(e_new_out);
                }
            }
        }
    }
    // for (auto [idx, e] : replace_e_new_in) {
    //     _edgeLabel_2_edge[curr_edgegp][idx] = e;
    // }
    for (auto [label, edges] : e_new_outs) {
        for (auto e : edges) {
            _edgeLabel_2_edge[label].push_back(e);
            _edges_num += 1;
        }
        this -> sort_edgeLabel_2_edge_by_tail(label);
    }
    /***********************************************
    ***  Relabel all nodes. After insertion point, push nodes 1 offset back.
    ***    Starting from the label with 1 less. (>=)!!!
    ************************************************/
    for (int i=0; i<_nodes_num; i++) {
        // 
        if ((_node_ptrs[i]+1) >= this->get_node_label(new_node) && (_node_ptrs[i] != this -> get_node_label(new_node))) {
            _node_ptrs[i] += 1;
        }
        // cout << "*_node_ptrs[i]: " << _node_ptrs[i] << endl;
        // cout << "   new        : " << this -> get_node_label(new_node) << endl;
    }

    /***********************************************
    ***  Sort the current label group after all nodes are updated.
    ************************************************/
    this -> sort_edgeLabel_2_edge_by_tail(curr_edgegp);
...
//     if (_node_2_innodes.find(new_node_name) == _node_2_innodes.end()) {
//         // Not found => the first appearance.
//         set<int> new_innodes_vc{node_1_name};
//         _node_2_innodes[node_2_name] = new_innodes_vc;
//     } else {
        ...
        if (edge != edges.begin()) {
            if (edge->get_tail_label() < prev(edge)->get_tail_label()) {
                ...
                if (verbose && !benchmark_mode) {
                    ...
                    edge -> print_edge();
                    this->get_innodesList_uniq(label, prev(edge)->get_head_name());
                    this->get_innodesList_uniq(label, edge->get_head_name());
                }
    ...
    *****************************************/
    for (auto& [label, edges] : _edgeLabel_2_edge) {
        ...
        for (auto& edge : edges) {
            ...
            ofile_name_DOT << "\t" << this->get_decoded_nodeName(edge.get_tail_name()) << " -> " << this->get_decoded_nodeName(edge.get_head_name())  << " [label=" << this->get_decoded_label(edge.get_label()) << "];" << endl;
